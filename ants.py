...
class Insect(object):
    ...
    def reduce_armor(self, amount):
        ...
        """
        self.armor -= amount
        if self.armor <= 0 and self.place:
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if self.direction == 'right'and self.place.name == 'Hive':
            return  #just stop
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place = self.place #starting from the place of the ant
        distance = 0
        ...
        while place is not hive and distance >= self.min_range and distance <= self.max_range:
            if place.bees:
    ...
    def action(self, colony):
        ...
        while place:
            ...
            if place and place.ant:
                ...
                if place.ant.is_container and place.ant.contained_ant and place.ant.contained_ant.buffed == False:
...
def apply_effect(effect, bee, duration):
    ...
    affected_action = effect(bee_action, bee)
    time = 0
    ...
    def insects_in_front(self, hive):
        ...
        while place != hive:
            if place.ant and place.ant.name != 'Laser':
                distance[place.ant] = cur_distance
                if place.ant.is_container and place.ant.contained_ant and place.ant.contained_ant.name != 'Laser':
                    distance[place.ant.contained_ant] = cur_distance
            if place.bees:
                for i in range(len(place.bees)):
                    distance[place.bees[i]] = cur_distance
            place = place.entrance
            cur_distance += 1
        return distance
        # END Problem OPTIONAL
