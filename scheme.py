...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        #print('procedure {} is evaluated!'.format(first))
    ...
    #print('procedure {} is evaluated!'.format(procedure))

    #I defined this function because the map method only works for one-argument function, but
    #scheme_eval takes in two arguments
    if isinstance(procedure, MacroProcedure):
        return scheme_eval(procedure.apply_macro(rest, env), env) 
    def operand_eval(expr, env = env, _ = None):
        return scheme_eval(expr, env)

    rest = rest.map(operand_eval)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('The length of formals and vals do not match!')
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    new_frame = env.make_child_frame(nil, nil)
    while bindings != nil:
        pair = bindings.first
        check_form(pair, 2, 2)
        if not scheme_symbolp(pair.first):
            raise SchemeError('{} is not a symbol!'.format(pair.first))
        new_frame.define(pair.first, scheme_eval(pair.second.first, env))
...
def do_define_macro(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    target = expressions.first
    if not (isinstance(target, Pair) and scheme_symbolp(target.first) and expressions.second):
        raise SchemeError('invalid syntax of define-macro!')
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        """
        #print('the expr is {}, tail is {}'.format(expr, tail))
